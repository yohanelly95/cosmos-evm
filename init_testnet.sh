...
KEY2="bob"
KEY3="charlie"
CHAINID="razor_12938912-1"
MONIKER="init-validator"
KEYRING="test"
...
# Change parameter token denominations to qrzr
cat $HOME/.razord/config/genesis.json | jq '.app_state["staking"]["params"]["bond_denom"]="qrzr"' > $HOME/.razord/config/tmp_genesis.json && mv $HOME/.razord/config/tmp_genesis.json $HOME/.razord/config/genesis.json
cat $HOME/.razord/config/genesis.json | jq '.app_state["crisis"]["constant_fee"]["denom"]="qrzr"' > $HOME/.razord/config/tmp_genesis.json && mv $HOME/.razord/config/tmp_genesis.json $HOME/.razord/config/genesis.json
cat $HOME/.razord/config/genesis.json | jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="qrzr"' > $HOME/.razord/config/tmp_genesis.json && mv $HOME/.razord/config/tmp_genesis.json $HOME/.razord/config/genesis.json
cat $HOME/.razord/config/genesis.json | jq '.app_state["evm"]["params"]["evm_denom"]="qrzr"' > $HOME/.razord/config/tmp_genesis.json && mv $HOME/.razord/config/tmp_genesis.json $HOME/.razord/config/genesis.json
cat $HOME/.razord/config/genesis.json | jq '.app_state["inflation"]["params"]["mint_denom"]="qrzr"' > $HOME/.razord/config/tmp_genesis.json && mv $HOME/.razord/config/tmp_genesis.json $HOME/.razord/config/genesis.json
...
razord add-genesis-account $KEY1 1050000000000000000000000000qrzr --keyring-backend $KEYRING
razord add-genesis-account $KEY2 1000000000000000000000000000qrzr --keyring-backend $KEYRING
razord add-genesis-account $KEY3 1000000000000000000000000000qrzr --keyring-backend $KEYRING
...
razord gentx $KEY1 900000000000000000000000qrzr --keyring-backend $KEYRING --chain-id $CHAINID --output-document $HOME/.razord/config/gentx/gentx-1.json
#razord gentx $KEY2 1000000000000000000000qrzr --keyring-backend $KEYRING --chain-id $CHAINID
...
#razord start --pruning=nothing --trace --log_level trace --minimum-gas-prices=1.000qrzr --json-rpc.api eth,txpool,personal,net,debug,web3 --rpc.laddr "tcp://0.0.0.0:26657" --api.enable true --api.enabled-unsafe-cors true
